...
**剧情密度先于字数裁剪**。字数受限时，优先减少本章承载的剧情量（合并场景、推迟次要 beats 到下一章），而非压缩每个 beat 的展开空间。一个 beat 如果在本章字数预算内无法完成"铺垫→发展→收束"的完整节奏，就推迟到下一章——不在本章硬塞。

**必要铺垫保护清单**（以下内容不得为凑字数砍掉）：
- 角色对新信息的**第一反应**（震惊、怀疑、沉思等，至少一句话）
- 对话之间的**过渡和潜台词**（不能一个人说完另一个人无缝接上）
- 新场景的**环境交代**（时间、地点、氛围，至少一句）
- 角色做决定前的**思考过程**（至少一个转折/犹豫）
- **章末余韵**（不能以对话尾句硬截断作结尾）

**字数越界的正确处理方式**：先判断是否承载了过多 plot beats。如果是，优先删除末尾未完成的 beat 推迟到下一章开头，而不是压缩现有 beat 的展开。真正的超限（非承载量问题）才合并次要场景。不要把角色反应、情绪积累、环境氛围当作"次要铺垫"裁切。

短字数章的写法不是把长章写完再修边，而是先控制承载量：1200-1600 字通常只写 2-3 个场景、1 个主转折、1 个章末钩子。
